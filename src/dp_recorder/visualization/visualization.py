...
def _summarize_val(val: Any) -> str:
    ...
    if isinstance(val, np.ndarray):
        ...
        if val.size == 0:
            return f"Empty Array {shape_str}"
        if val.size < 5:
            return f"{val} {shape_str}"
    ...
    for k, v in params.items():
        if k in keys:
            ...
            if k == "epsilon":
                ...
                symbol = "δ"
            elif k in ["noise_multiplier", "sigma"]:
                symbol = "σ"
...
def render_flow_graph(
    ...
    *,
    ...
    for i, entry in enumerate(rec.log):
        ...
        is_crash_site = (i == crash_point) and (
            rec.mode == "replay" or rec.mode == rec.mode.REPLAY
        )
        ...
        if param_str:
            header_text += (
                f"<br/><font point-size='10' color='{font_color}'>[{param_str}]</font>"
            )
        ...
        val_dp = (
            list(entry.inputs_dp.values())[0]
            if (entry.inputs_dp and is_replayed)
            else None
        )
    ...
    dot.edge(
        prev_node_id, "end", style="dashed" if rec._cursor < len(rec.log) else "solid"
    )
...
def plot_call_diffs(
    ...
    *,
    ...
    ax.bar(
        df["Mechanism"],
        df["Sensitivity Limit"],
        color="gray",
        alpha=0.2,
        label="Allowed Sensitivity (Limit)",
        width=0.6,
    )
    ...
    ax.legend(
        custom_lines, ["Allowed Limit", "Pass (Dist ≤ Sens)", "Violation (Dist > Sens)"]
    )
